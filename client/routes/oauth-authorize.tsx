...
import { fetchSessionInfo, type SessionInfo, type SessionStatus, } from '#client/session.ts';
...
export function OAuthAuthorizeRoute(handle: Handle) {
    ...
    async function submitDecision(decision: 'approve' | 'deny', form?: HTMLFormElement) {
        if (submitting)
            return;
        submitting = true;
        handle.update();
    ...
    async function handleSubmit(event: SubmitEvent) {
        ...
        await submitDecision('approve', hasSession ? undefined : event.currentTarget);
    }
    return () => {
        ...
        const scopeLabel = scopes.length > 0 ? scopes.join(', ') : 'No scopes requested.';
        const sessionEmail = session?.email ?? '';
        const isSessionReady = sessionStatus === 'ready';
        const isSessionLoading = sessionStatus === 'loading' || sessionStatus === 'idle';
        ...
        return (<section mix={[
            ...
            css({ margin: 0, color: colors.textMuted })
				...
				{isSessionLoading ? (<p mix={[
                css({ color: colors.textMuted })
            ]}>Checking your session…</p>) : null}
				{isLoggedIn ? (<section mix={[
                css({
                    padding: spacing.md,
				...
				{message ? (<p role={message.type === 'error' ? 'alert' : undefined} mix={[
                css({
            ...
            on<HTMLFormElement, "submit">("submit", handleSubmit)
                ...
                css({
                    ...
                    fontSize: typography.fontSize.sm,
								...
								<input type="email" name="email" required autoComplete="email" placeholder="you@example.com" disabled={actionsDisabled} mix={[
                css({
                    padding: spacing.sm,
                    borderRadius: radius.md,
                    border: `1px solid ${colors.border}`,
                ...
                css({
                    ...
                    fontSize: typography.fontSize.sm,
								...
								<input type="password" name="password" required autoComplete="current-password" placeholder="Enter your password" disabled={actionsDisabled} mix={[
                css({
                    padding: spacing.sm,
                    borderRadius: radius.md,
                    border: `1px solid ${colors.border}`,
            ...
            css({ display: 'flex', gap: spacing.sm, flexWrap: 'wrap' })
						...
						<button type="submit" disabled={actionsDisabled} mix={[
            css({
                ...
                opacity: actionsDisabled ? 0.7 : 1,
						...
						<button type="button" disabled={actionsDisabled} mix={[
            css({
                padding: `${spacing.sm} ${spacing.lg}`,
                borderRadius: radius.full,
                border: `1px solid ${colors.border}`,
            ...
            on<HTMLButtonElement, "click">("click", () => submitDecision('deny'))
        ]}>
							Deny
						</button>
					</div>
